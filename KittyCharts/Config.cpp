...
void Config::loadFile(const std::string& configFilepath)
{
    ...
    try {
        ...
        viewer.enable(pt.get<bool>("Viewer.enabled"));
        viewer.port(pt.get<unsigned>("Viewer.port"));
        viewer.timeout(pt.get<unsigned>("Viewer.timeout"));
        viewer.interval(pt.get<unsigned>("Viewer.interval"));
    }
